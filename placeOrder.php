...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['buy_book'])) {
    ...
    $order_query = "INSERT INTO carts ( customer_username, customer_phone, customer_address, seller_username, book_name, author_name, images, quantity, prices, status, created_at, updated_at)
    ...
    if ($conn->query($order_query) === TRUE) {
        ...
        header("Location: welcome.php?success=1&message=" . urlencode($success_message));
        exit();
    } else {
