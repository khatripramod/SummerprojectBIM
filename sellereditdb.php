...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $customerQuery = "SELECT * FROM sellers WHERE username = ?";
    ...
    if ($userResult->num_rows > 0 && $customerResult->num_rows > 0) {
        ...
        if (isset($_POST['update'])) {
            ...
            $revenue = $_POST['revenue_generated'];
            $email = $_POST['email'];
            ...
            $updateCustomerQuery = "UPDATE sellers SET full_name = ?, email = ?, gender = ?, phone_number = ? , address=?, revenue_generated = ? WHERE username = ?";
            $updateCustomerStmt = $conn->prepare($updateCustomerQuery);
            $updateCustomerStmt->bind_param("sssssss", $fullName,  $email, $gender, $phoneNumber, $addresss, $revenue, $username);
        ...
        if (isset($_POST['delete'])) {
            ...
            $deleteCustomerQuery = "DELETE FROM sellers WHERE username = ?";
